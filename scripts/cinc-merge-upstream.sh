...
# A merge that stops on conflicts leaves MERGE_HEAD and is handled below. One
# that fails without it never started, so report it instead of falling through
# to the "already up to date" branch and exiting 0 having merged nothing.
merge_failed=""
merge_output="$(git merge --no-commit --no-ff "$TEMP_BRANCH" 2>&1)" || merge_failed=yes
if [ -n "$merge_failed" ] && ! git rev-parse --verify MERGE_HEAD >/dev/null 2>&1; then
  echo "$merge_output" >&2
  echo "error: merging $UPSTREAM_REF into $current_branch failed; nothing was merged." >&2
  exit 1
fi
