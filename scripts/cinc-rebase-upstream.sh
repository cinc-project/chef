...
# that should not appear in the Cinc fork. On chef-18 that was the omnibus
# privatization commit; upstream main carries no omnibus/ at all, so there
# is no default skip commit -- pass shas to drop if upstream ever grows one.
#
# Usage:
...
#   scripts/cinc-rebase-upstream.sh upstream/main abcdef12
...
echo "==> Skipping:    ${SKIP_SHAS[*]:-(nothing)}"
