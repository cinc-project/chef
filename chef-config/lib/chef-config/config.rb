...
module ChefConfig
  ...
  class Config
    ...
    def self.etc_chef_dir(is_windows = ChefUtils.windows?)
      path = is_windows ? c_chef_dir : PathHelper.join("/etc", ChefUtils::Dist::Infra::DIR_SUFFIX)
      PathHelper.cleanpath(path)
    end

    # On *nix, /var/chef
    def self.var_chef_dir(is_windows = ChefUtils.windows?)
      path = is_windows ? c_chef_dir : PathHelper.join("/var", ChefUtils::Dist::Infra::DIR_SUFFIX)
      PathHelper.cleanpath(path)
    end
    ...
    def self.c_chef_dir
      ...
      path = PathHelper.join(drive, ChefUtils::Dist::Infra::DIR_SUFFIX)
      PathHelper.cleanpath(path)
    end
