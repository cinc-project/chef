  ...
  module Compliance
    class Runner < EventDispatch::Base
      ...
      def run_started(run_status)
        # In order to make sure we have all the data w eneed to validate config,
        # we need to wait until the run_started event.  Some validations (such as run_id)
        # would fail if we started too soon (node_load_completed)
        self.run_id = run_status.run_id
        # We now have enough data to validate configuration.  Because the converge
        # is best coupled with the associated compliance run, these validations
      ...
      def report(report = generate_report)
        ...
        # because we want to ensure that it is visible in the output - and not lost in backscroll.
