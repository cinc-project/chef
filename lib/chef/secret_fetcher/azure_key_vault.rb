...
class Chef
  class SecretFetcher
    ...
    # name, such as "myvault/secretkey1".
    #
    # @example
    #
    # fetcher = SecretFetcher.for_service(:azure_key_vault, { vault: "myvault" }, run_context )
    # fetcher.fetch("secretkey1", "v1")
    #
    # @example
    #
    # fetcher = SecretFetcher.for_service(:azure_key_vault, {}, run_context )
    # fetcher.fetch("myvault/secretkey1", "v1")
    class AzureKeyVault < Base
      ...
      # If it is not in the provided name in the form "vaultname/secretname"
      # it will determine the vault name from `config[:vault]`.
      # @param name [String] the secret name or vault and secret name in the form "vaultname/secretname"
